...
#-----------------------------------------------------------------DURATION OPTIMISATION---------------------------------------------
    ...
    if optiond == 'Random Forest Updated':
        if displayd == 'Accuracy':
            rfopti_scored=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\rfopti_scored")
            st.write(rfopti_scored)
        elif displayd == 'Classification_report':
            rfopti_classificationd=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\rfopti_classificationd")
            st.text(rfopti_classificationd)
        else:
            rfopti_matrixd=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\rfopti_matrixd")
            st.dataframe(rfopti_matrixd)
    elif optiond == 'XG boost Updated':
        if displayd == 'Accuracy':
            xgopti_scored=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\xgopti_scored")
            st.write(xgopti_scored)
        elif displayd == 'Classification_report':
            xgopti_classificationd=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\xgopti_classificationd")
            st.text(xgopti_classificationd)
        else:
            xgopti_matrixd=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\xgopti_matrixd")
            st.dataframe(xgopti_matrixd)
    
    rfopti_score=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\rfopti_score")
    rfopti_scored=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\rfopti_scored")
    xgopti_score=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\XGBoost_opti_score")
    xgopti_scored=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\xgopti_scored")
...
if page==pages[4]:
    ...
    kmeans = joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\kmeans")
    image = Image.open(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\US clusters.png")     #chemin de votre image à définir
...
# PREPROCESSING DE L'OBSERVATION----------------------------------------------------------------------------------------------------------------   
    ...
    if st.checkbox("Afficher la prédiction"):
        clf = joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\XGBoost")                         # pour les deux cibles, nous avons utilisé le xgboost dans le streamlit car cela consomme moins de ressource
        clfd=joblib.load(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\Duration\XGBoostd")
...
if page==pages[5]:
    ...
    image = Image.open(r"C:\Users\mbobe\Desktop\Dossier APEC-DATA analyst\FORMATION DATA\FIRE\FIRE_SIZE\planet.jpg")     #chemin de votre image à définir
    st.image(image, use_column_width=True)
